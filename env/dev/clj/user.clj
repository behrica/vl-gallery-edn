(ns user
  ...
  (:require
    ...
    [behrica.vl-gallery-edn.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (behrica.vl-gallery-edn.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (behrica.vl-gallery-edn.config/system-config {:profile :test})
                                  (ig/prep)))))
